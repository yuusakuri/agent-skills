...
|---------|------|---------|--------|------|---------|
| 2.2.0 | 2026-07-05 | v2.31.0 | WS-Z5 | minor | Reciprocal When NOT to Use pointer to `customer-journey-map`; collision pair declared with new trigger fixtures. |
...
The WS-Z5 fixture backfill declared `customer-journey-map` as a new collision pair for this skill in `scripts/trigger-eval-roster.yaml`, but the reciprocal "When NOT to Use" pointer was never added. The enforcing `check-reciprocal-boundary-pointers` gate caught the gap. Adds one bullet pointing to `customer-journey-map` for the case where synthesized findings are being mapped onto a customer journey. No other content change.
