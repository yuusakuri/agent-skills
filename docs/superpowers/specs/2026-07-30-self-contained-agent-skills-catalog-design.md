...
- Store all 97 adopted Skills in the public catalog repository.
...
`skills/` is the runtime-ready, canonical snapshot. It contains the 96 pinned external Skills and the locally maintained `document-architecture` Skill.
...
4. Fetch each unique upstream repository and commit once.
5. Build the complete 97-Skill snapshot in a staging directory.
6. Preserve upstream license text in `third-party-licenses/`.
7. Validate declared Skill names, paths, counts, capabilities, and the local Skill.
8. Replace `skills/` only after the complete staged snapshot passes.
9. Run CI and review the vendored diff before release.
...
Vendoring external content makes license retention mandatory. `third-party-licenses/` will contain one directory per unique upstream repository, including its license text and pinned source metadata. CI will fail when a vendored upstream repository has no corresponding license record.
...
1. Generate and commit all 97 Skills under `skills/`.
...
- A missing upstream license blocks the update.
...
- exactly 97 unique Skill names are vendored;
- all five rejected duplicate candidates remain absent;
- every external Skill matches its pinned source;
- all six upstream repositories are fetched at most once per update;
- all six upstream license records exist;
